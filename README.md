...
**First run.** On the **Settings** tab, load `defaults.yaml` as your settings
file, edit the `accounts:` and `funds:` sections to match your troop's
TroopWebHost setup, and load it back. Or,
from a checkout of this repo, load `test/fixtures/sample-settings.yaml` with
...
It is YAML, and the **Settings** tab offers it under two extensions holding
byte-identical content: `troop-settings.yaml`, which says what the file is and
gets syntax highlighting in anything that knows the format, and
`troop-settings.txt`, which on Windows reliably opens in Notepad where a `.yaml`
may open in whatever happens to be registered, or in nothing. The people
inheriting this are volunteers, so both routes stay open. **Load settings file**
accepts either — the extension is never read, the content decides.
...
`defaults.yaml` is the starting template: the example chart of accounts, ready to
edit. `test/fixtures/sample-settings.yaml` pairs with `sample-export.csv` — load
...
defaults.yaml         starting settings file (generated)
...
test/fixtures/sample-settings.yaml  settings to pair with it (generated)
```
