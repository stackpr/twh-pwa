// csv.js — RFC 4180 parser + MD5. No dependencies.
...
export function parseCSV(text) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const rec = {};
    for (let cix = 0; cix < headers.length; cix++) {
      const v = (raw[cix] ?? '').trim();
      rec[headers[cix]] = v === '' ? null : v;
    }
    records.push(rec);
  }
...
/* ---- MD5 (public-domain style compact implementation) ---- */
...
function md5blk(s) { const md5blks = []; for (let i = 0; i < 64; i += 4)
...
export function md5(str) {
  // UTF-8 encode to a binary string so non-ASCII names hash deterministically
